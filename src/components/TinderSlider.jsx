      ...
      <View style={styles.card}>
        {imageErrors[card.id] ? (
          ...
          <Image
            source={card.image}
            style={styles.image}
            resizeMode="stretch" // Changed from "stretch" to "cover" for better appearance
            onError={() => handleImageError(card.id)}
          />
        )}
      </View>
  ...
  return (
    <View style={styles.container}>
      <Swiper
        ...
        renderCard={renderCard}
        onSwiped={onSwiped}
        cardIndex={currentIndex % cards.length} // Ensures smooth looping
        stackSize={3}
        infinite
        backgroundColor="transparent"
        cardVerticalMargin={10}
        ...
        overlayOpacityHorizontalThreshold={10} // Makes swipes more precise
        overlayLabels={{
          left: {
            title: "NOPE",
            style: {
